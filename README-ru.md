...
| `WHISPER_WORD_TIMESTAMPS` | При значении `true` глобально включает пословные метки времени. В выводе `verbose_json` каждый сегмент будет содержать массив `words` с временем начала/конца и вероятностью для каждого слова. Также можно включить для отдельного запроса. | *(не задан)* |
...
| `word_timestamps` | логическое | — | Извлечь пословные метки времени. При `true` в выводе `verbose_json` каждый сегмент содержит массив `words`. По умолчанию: `false` (или переменная `WHISPER_WORD_TIMESTAMPS`). |
...
При `word_timestamps=true` каждый сегмент в ответе `verbose_json` содержит массив `words`:
