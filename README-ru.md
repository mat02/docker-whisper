...
**1. Установите API-ключ.** Сгенерируйте надёжный случайный ключ и задайте `WHISPER_API_KEY` в `env`-файле. После этого все запросы должны содержать `Authorization: Bearer <ключ>`.
